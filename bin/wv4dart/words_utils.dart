...
class WordsUtils {
    ...
    static List<int> ones_count_table = 
    [
    ...
    {
        List<int> byteptr = wpmd.data; //byteptr needs to be unsigned chars, so convert to int array
        List<int> b_array = new List<int>(12);
        int i = 0;
        ...
        w.c[0].median[0] = exp2s(WvDartUtils.getS16IntFromBytes(b_array[1], b_array[0]));
        w.c[0].median[1] = exp2s(WvDartUtils.getS16IntFromBytes(b_array[3], b_array[2]));
        w.c[0].median[2] = exp2s(WvDartUtils.getS16IntFromBytes(b_array[5], b_array[4]));
        ...
        if ((wps.wphdr.flags & (Defines.MONO_FLAG | Defines.FALSE_STEREO)) == 0)
        {
            ...
            w.c[1].median[0] = exp2s(WvDartUtils.getS16IntFromBytes(b_array[7], b_array[6]));
            w.c[1].median[1] = exp2s(WvDartUtils.getS16IntFromBytes(b_array[9], b_array[8]));
            w.c[1].median[2] = exp2s(WvDartUtils.getS16IntFromBytes(b_array[11], b_array[10]));
        }
    ...
    static int read_hybrid_profile(WavpackStream wps, WavpackMetadata wpmd)
    {
        ...
        if ((wps.wphdr.flags & Defines.HYBRID_BITRATE) != 0)
        {
            ...
            wps.w.c[0].slow_level = exp2s(WvDartUtils.getS16IntFromBytes(uns_buf_plusone, uns_buf));
            ...
            if ((wps.wphdr.flags & (Defines.MONO_FLAG | Defines.FALSE_STEREO)) == 0)
            {
                uns_buf = byteptr[buffer_counter] & 0xff;
                uns_buf_plusone = byteptr[buffer_counter + 1] & 0xff;
                wps.w.c[1].slow_level = exp2s(WvDartUtils.getS16IntFromBytes(uns_buf_plusone, uns_buf));
                buffer_counter = buffer_counter + 2;
            }
        }
        ...
        if (buffer_counter < bytecnt)
        {
            ...
            wps.w.bitrate_delta[0] = exp2s(WvDartUtils.getS16IntFromBytes(uns_buf_plusone, uns_buf));
            buffer_counter = buffer_counter + 2;
            ...
            if ((wps.wphdr.flags & (Defines.MONO_FLAG | Defines.FALSE_STEREO)) == 0)
            {
                uns_buf = byteptr[buffer_counter] & 0xff;
                uns_buf_plusone = byteptr[buffer_counter + 1] & 0xff;
                wps.w.bitrate_delta[1] = exp2s(WvDartUtils.getS16IntFromBytes(uns_buf_plusone, uns_buf));
                buffer_counter = buffer_counter + 2;
            }
    ...
    static words_data update_error_limit(words_data w, int flags)
    {
    ...
    static int get_words(int nsamples, int flags, words_data w, Bitstream bs, List<int> buffer)
    {
        List<entropy_data> c = w.c;
        int csamples;
        int buffer_counter = 0;
        int entidx = 1;
    ...
    static int count_bits(int av)
    {
        ...
        else
        {
            if (av < (1 << 16))
            {
                return nbits_table[BitsUtils.zeroFillRightShift(av, 8)] + 8; // Unsigned right shift!!
            }
            else
            {
                if (av < (1 << 24))
                {
                    return nbits_table[BitsUtils.zeroFillRightShift(av, 16)] + 16; // Unsigned right shift!!
                }
                else
                {
                    return nbits_table[BitsUtils.zeroFillRightShift(av, 24)] + 24; // Unsigned right shift!!
                }
            }
        }
    }
    ...
    static int read_code(Bitstream bs, int maxcode)
    {
    ...
    static int restore_weight(int weight)
    {
        int result;
