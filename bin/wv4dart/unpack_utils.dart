library wvunpackutils;

import "types.dart";
import "bits_utils.dart";
import "meta_data_utils.dart";
...
class UnpackUtils {
    ...
    static int read_decorr_weights(WavpackStream wps, WavpackMetadata wpmd)
    {
        ...
        while (termcnt > 0)
        {
            ...
            dpp.weight_A = WordsUtils.restore_weight( byteptr[counter]);
            ...
            if ((wps.wphdr.flags & (Defines.MONO_FLAG | Defines.FALSE_STEREO)) == 0)
            {
                dpp.weight_B = WordsUtils.restore_weight( byteptr[counter]);
                counter++;
            }
    ...
    static int read_decorr_samples(WavpackStream wps, WavpackMetadata wpmd)
    {
        ...
        while (counter < wpmd.byte_length)
        {
            if (dpp.term > Defines.MAX_TERM)
            {
                ...
                dpp.samples_A[0] = WordsUtils.exp2s((uns_buf0 + (uns_buf1 << 8)));
                dpp.samples_A[1] = WordsUtils.exp2s((uns_buf2 + (uns_buf3 << 8)));
                counter += 4;
                ...
                if ((wps.wphdr.flags & (Defines.MONO_FLAG | Defines.FALSE_STEREO)) == 0)
                {
                    ...
                    dpp.samples_B[0] = WordsUtils.exp2s((uns_buf0 + (uns_buf1 << 8)));
                    dpp.samples_B[1] = WordsUtils.exp2s((uns_buf2 + (uns_buf3 << 8)));
                    counter += 4;
                }
            }
            else if (dpp.term < 0)
            {
                ...
                dpp.samples_A[0] = WordsUtils.exp2s((uns_buf0 + (uns_buf1 << 8)));
                dpp.samples_B[0] = WordsUtils.exp2s((uns_buf2 + (uns_buf3 << 8)));
            ...
            else
            {
                ...
                while (cnt > 0)
                {
                    ...
                    int input = WvDartUtils.getS16IntFromBytes(uns_buf1, uns_buf0);
                    
                    dpp.samples_A[m] = WordsUtils.exp2s(input);
                    counter += 2;
                    ...
                    if ((wps.wphdr.flags & (Defines.MONO_FLAG | Defines.FALSE_STEREO)) == 0)
                    {
                        ...
                        dpp.samples_B[m] = WordsUtils.exp2s(input);
                        counter += 2;
                    }
    ...
    static int unpack_samples(WavpackContext wpc, List<int> buffer, int sample_count)
    {
        WavpackStream wps = wpc.stream;
        ...
        int mute_limit = ((1 << ((flags & Defines.MAG_MASK) >> Defines.MAG_LSB)) + 2);
        decorr_pass dpp;
        int tcount;
        ...
        if ((flags & (Defines.MONO_FLAG | Defines.FALSE_STEREO)) > 0)
        {
            ...
            // TODO void setRange(int start, int length, List<E> from, [int startFrom]);
            //System.arraycopy(temp_buffer, 0, buffer, 0, sample_count);
            buffer.setRange(0, sample_count, temp_buffer, 0);
        ...
        else
        {
            ...
            // TODO
            //System.arraycopy(temp_buffer, 0, buffer, 0, (sample_count*2));
            buffer.setRange(0, (sample_count*2), temp_buffer, 0);
    ...
    static void decorr_stereo_pass(decorr_pass dpp, List<int> buffer, int sample_count, int buf_idx)
    {
        int delta = dpp.delta;
        ...
        switch (dpp.term)
        {
            case 17:
                for (bptr_counter = buf_idx; bptr_counter < buf_idx + sample_count * 2; bptr_counter += 2)
                {
                    ...
                    dpp.samples_A[0] = ((weight_A * sam_A + 512) >> 10) + buffer[bptr_counter];
                    ...
                    dpp.samples_B[0] = ((weight_B * sam_A + 512) >> 10) + buffer[bptr_counter + 1];
            ...
            case -1:
                for (bptr_counter = buf_idx; bptr_counter < buf_idx + sample_count * 2; bptr_counter += 2)
                {
                    sam_A = buffer[bptr_counter] + ((weight_A * dpp.samples_A[0] + 512) >> 10);
                    ...
                    dpp.samples_A[0] = buffer[bptr_counter + 1] + ((weight_B * sam_A + 512) >> 10);
            ...
            case -2:
                ...
                for (bptr_counter = buf_idx; bptr_counter < buf_idx + sample_count * 2; bptr_counter += 2)
                {
                    sam_B = buffer[bptr_counter + 1] + ((weight_B * dpp.samples_B[0] + 512) >> 10);
                    ...
                    dpp.samples_B[0] = buffer[bptr_counter] + ((weight_A * sam_B + 512) >> 10);
            ...
            default:
                ...
                sam_A = 0;
                m = 0; k = dpp.term & (Defines.MAX_TERM - 1);
                for (bptr_counter = buf_idx; bptr_counter < buf_idx + sample_count * 2; bptr_counter += 2)
                {
                    sam_A = dpp.samples_A[m];
                    dpp.samples_A[k] = ((weight_A * sam_A + 512) >> 10) + buffer[bptr_counter];
                    ...
                    dpp.samples_B[k] = ((weight_B * sam_A + 512) >> 10) + buffer[bptr_counter + 1];
                ...
                if (m != 0)
                {
                    List<int> temp_samples = new List<int>(Defines.MAX_TERM);
                    ...
                    temp_samples.setRange(0, dpp.samples_B.length, dpp.samples_B, 0);
    ...
    static void decorr_stereo_pass_cont(decorr_pass dpp, List<int> buffer, int sample_count, int buf_idx)
    {
        ...
        int end_index = buf_idx + sample_count * 2;
        ...
        switch (dpp.term)
        {
            case 17:
                for (buffer_index = buf_idx; buffer_index < end_index; buffer_index += 2)
                {
                    ...
                    buffer[buffer_index] = ((weight_A * sam_A + 512) >> 10)
                    ...
                    buffer[buffer_index + 1] = ((weight_B * sam_A + 512) >> 10)
            ...
            case 18:
                for (buffer_index = buf_idx; buffer_index < end_index; buffer_index += 2)
                {
                    // TODO: itt bazinagy lesz a szám! valami nem jó
            ...
            case -1:
                for (buffer_index = buf_idx; buffer_index < end_index; buffer_index += 2)
                {
                    buffer[buffer_index] = ((weight_A * buffer[buffer_index - 1] + 512) >> 10)
                    ...
                    buffer[buffer_index + 1] = ((weight_B * buffer[buffer_index] + 512) >> 10)
            ...
            case -2:
                ...
                for (buffer_index = buf_idx; buffer_index < end_index; buffer_index += 2)
                {
                    ...
                    buffer[buffer_index + 1] = ((weight_B * buffer[buffer_index - 2] + 512) >> 10)
                        + (sam_A = buffer[buffer_index + 1]);
            ...
            default:
                ...
                for (buffer_index = buf_idx; buffer_index < end_index; buffer_index += 2)
                {
                    buffer[buffer_index] = ((weight_A * buffer[tptr] + 512) >> 10)
                        + (sam_A = buffer[buffer_index]);

                    if (buffer[tptr] != 0 && sam_A != 0)
                        weight_A += (((buffer[tptr] ^ sam_A) >> 30) | 1) * delta;

                    buffer[buffer_index + 1] = ((weight_B * buffer[tptr + 1] + 512) >> 10)
                        + (sam_A = buffer[buffer_index + 1]);
    ...
    static void decorr_mono_pass(decorr_pass dpp, List<int> buffer, int sample_count, int buf_idx)
    {
        ...
        switch (dpp.term)
        {
            ...
            default:
                ...
                if (m != 0)
                {
                    ...
                    temp_samples.setRange(0, dpp.samples_A.length, dpp.samples_A, 0);
    ...
    static List<int> fixup_samples(WavpackStream wps, List<int> buffer, int sample_count)
    {
        int flags = wps.wphdr.flags;
        int shift = ((flags & Defines.SHIFT_MASK) >> Defines.SHIFT_LSB);
